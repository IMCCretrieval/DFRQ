...
from semi_utils.cifar10 import *
...
def _ssl_fft_beta(data,rank_num):
    ...
    r=0.1*(rank_num)
    #r=0.3*(rank_num)  #cuda=2
    #r=0.2*(rank_num)  #cuda=3
    ...
    if rank_num>0:
        ...
        for j in range(3):
            ...
            #swap
            # amp[:,x1:x2,y1:y2,j]=(1.0-rank_num*0.2)*amp[:,x1:x2,y1:y2,j]+(rank_num*0.2)*amp[indices,x1:x2,y1:y2,j]
            ...
            # pha[:,x1:x2,y1:y2,j]=(0.8-rank_num*0.2)*pha[:,x1:x2,y1:y2,j]+(0.2+rank_num*0.2)*pha[indices,x1:x2,y1:y2,j] #beta
            #pha[:,x1:x2,y1:y2,j]=(1.0-rank_num*0.3)*pha[:,x1:x2,y1:y2,j]+(rank_num*0.3)*pha[indices,x1:x2,y1:y2,j] #cuda=3
            #pha[:,x1:x2,y1:y2,j]=(1.0-rank_num*0.1)*pha[:,x1:x2,y1:y2,j]+(rank_num*0.1)*pha[indices,x1:x2,y1:y2,j] #cuda=2
        #ifft
        for j in range(3):
            for i in range(data_num):
...
def avr_loss(diff, n_book,bn_word,batch_size): #输入是weights
