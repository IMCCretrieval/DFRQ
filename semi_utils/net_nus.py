from semi_utils.Functions_nus import *
...
with tf.variable_scope("placeholder"):
    ...
    label_o = tf.placeholder(tf.float32, shape=[None], name='label_o')   
    ...
    alpha = tf.placeholder(tf.float32,shape=[1], name='alpha')
...
#                                    initializer=initializer, trainable=True)#12*12=144-d  16 144
        self.Z = tf.get_variable('Z', [intn_word, len_code * n_book], dtype=tf.float32,
                                   initializer=tf.orthogonal_initializer(), trainable=True)#12*12=144-d  16 144

        self.Prototypes = tf.get_variable('C', [n_CLASSES, len_code * n_book], dtype=tf.float32,
                                         initializer=tf.orthogonal_initializer(), trainable=True)# 10 144
