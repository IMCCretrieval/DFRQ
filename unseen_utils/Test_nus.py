...
def color_deprocessing_toshow(x_):
    ...
    x_[:, :, 0] = (x_[:, :, 0]* 71.349 + 111.804)/255
    x_[:, :, 1] = (x_[:, :, 1]*  67.951 + 107.83)/255
    x_[:, :, 2] = (x_[:, :, 2]* 73.239 + 99.814)/255

    return x_

# Do image retrieval using PQ table
def PQ_retrieval(sess, x, training_flag, feature, Z, n_book, db_x, test_x, label_Similarity, visual_flag, TOP_K=5000):
    print("Do retrieval")
    pre_index = 0
    test_pre_index = 0
    iteration = 1000
    ...
    if visual_flag == True:
        ...
        fig.tight_layout()
        plt.savefig('./retrieval_results/nuswide/retrieval_result_%d.png'%((rnd_id)))
        plt.close()
